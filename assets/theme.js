...
theme.ProductForm = function (context, events, Product) {
  ...
  let isCarted = false,
    addonPreviousValues = {},
    ProductDisplayMessage = document.querySelector("inventory-message").children[1].innerText ? document.querySelector("inventory-message").children[1].innerText : '',
    MaincartNote = '';
  ...
  async function appendToCartNote(cartNote) {
    try {
      const res = await fetch('/cart.js');
      const cart = await res.json();

      const currentNote = cart.note || "";
      let updatedNote = currentNote ? `${currentNote}\n${cartNote}` : cartNote;

      const validNotes = [];

      const allNotes = updatedNote.split('\n');
      for (const note of allNotes) {
        const isValidNote = cart.items.some(item => {
          const variantParts = item.variant_title ? item.variant_title.split(' / ') : [];
          const variantSuffix = item.variant_title ? ` - ${item.variant_title}` : '';
          const baseTitle = item.title.endsWith(variantSuffix) ? item.title.slice(0, -variantSuffix.length) : item.title;
          const noteToMatch = [baseTitle, ...variantParts].filter(Boolean).join(' | ');
          return note.includes(noteToMatch);
        });

        if (isValidNote && !validNotes.includes(note)) {
      ...
      console.log("✅ Cart note updated:", updateData.note);
    } catch (err) {
  ...
  async function OfferedProductCarted(vid, qty, note) {
    if (typeof isCarted === "undefined" || !isCarted) {
      ...
      const addRes = await addToCart(items);
      const noteRes = await appendToCartNote(note);

      return addRes && noteRes;
    }
    return false;
  }
    ...
    addonItems.forEach((item) => {
      ...
      if (groupName.includes(selectedValue)) {
        ...
        inputField.addEventListener("change", () => {
          ...
          Offered_product_name = item.querySelector("span")?.innerText.trim();
          previousValues[item.dataset.addonVariantId] = inputField.value;
          updateAddonData(item.dataset.addonVariantId, inputField.value);
          checkAndUpdateInventoryMessage(base_variant_selected, Offered_product_name);
        });
  ...
  function handlePlusClick(event, previousValues) {
    Offered_product_name = event.target
      .closest(".addons-products__item")
  ...
  function checkAndUpdateInventoryMessage(variant, addonproduct) {
    if (!variant || !window.messageVariantFormat) return;

    let displayMessage = '';
    let relativeColor = '';

    const formattedOption1 = variant.option1
      .toLowerCase()
      .replace(/ /g, '_')
      .replace(/\s?\(\s?/g, '(')
      .replace(/\s?\)\s?/g, ')');
    ...
    if (variantFormat) {
      ...
      if (levelZeroMatch) {
        if (levelZeroMatch.conditional_handle && Product.handle.includes(levelZeroMatch.conditional_handle)) {
          displayMessage = levelZeroMatch.priority_message;
        } else {
      ...
      if (variant.option3 !== null) {
        const levelTwoMatch = variantFormat.level_two.find(item => item.option_name === variant.option3);
        if (levelTwoMatch) {
          displayMessage = levelTwoMatch.message;
          relativeColor = levelTwoMatch.color;
        }
      }

      if (addonproduct) {
        const previousMessage = displayMessage;
        const previousColor = relativeColor;

        const levelThreeMatch = variantFormat.level_three.find(item => item.option_name === addonproduct);
        if (levelThreeMatch) {
          displayMessage = levelThreeMatch.message;
          relativeColor = levelThreeMatch.color;
        } else {
          displayMessage = previousMessage;
          relativeColor = previousColor;
        }
      }
    }

    updateInventoryMessage(displayMessage, relativeColor);
  }



  function updateInventoryMessage(displayMessage, relativeColor) {
    const inventoryMessage = document.querySelector("inventory-message");

    if (!inventoryMessage) return false;

    const messageContainer = inventoryMessage.querySelector(".inventory__message");

    if (messageContainer) {
      const signal = messageContainer.querySelector(".inventory__signal");
      if (signal && relativeColor) {
        signal.style.backgroundColor = relativeColor;
      }

      const text = messageContainer.querySelector(".inventory__text");
      if (text && displayMessage) {
        text.textContent = displayMessage;
      }
    }

    if (displayMessage && relativeColor) {
      inventoryMessage.style.display = "block";
    } else {
      inventoryMessage.style.display = "none";
    }

    ProductDisplayMessage = displayMessage;
  ...
  new Shopify.OptionSelectors("product-select-" + Product.id, {
    ...
    onVariantSelected: function (variant) {
      ...
      base_variant_selected = variant;
      events.on("variantchange", function (variant) {
        checkAndUpdateInventoryMessage(variant, Offered_product_name);
      });
      ...
      const specification = document.querySelector("[data-product-dimension]");
      if (specification) {
    ...
    priceContainers.forEach(function (priceContainer) {
      ...
      events.on("variantchange", function (variant) {
        ...
        if (variant.compare_at_price > variant.price) {
          ...
          var price_save = ((compare_price - price) * 100.0) / compare_price;
          element.removeAttribute("style");
        }
    ...
    elements.forEach(function (element) {
      if (theme.settings.ajaxCartMethod == "drawer") {
        element.addEventListener("click", function (event) {
          ...
          OfferedProductCarted(Offered_product, Offered_product_qty, MaincartNote);
          Offered_product.pop();
        ...
        element.addEventListener("click", function (event) {
          ...
          OfferedProductCarted(Offered_product, Offered_product_qty, MaincartNote).then(
            (success) => {
              if (success) {
...
(function () {
  function collectionColorSwatch() {
    document.querySelectorAll(".product-index").forEach((el) => {
      const swatchItems = Array.from(el.querySelectorAll(".color__swatch li"));
      ...
      function setupSwatchSlider() {
        ...
        const contentWidth =
          swatchItems.length * (swatchItems[0]?.offsetWidth || 0) +
          swatchItems.length * 6;
      ...
      function findMatchingVariant(position, value) {
        ...
        return (
          el.querySelector(
            `.variant__image${selector}[data-option2="${defaultOptions.option2}"][data-option3="${defaultOptions.option3}"]`
          ) ||
          el.querySelector(
            `.variant__image${selector}[data-option2="${defaultOptions.option2}"]`
          ) ||
          el.querySelector(
            `.variant__image${selector}[data-option3="${defaultOptions.option3}"]`
          ) ||
          el.querySelector(`.variant__image${selector}`)
        );
      }

      function updateVariantOption(selectedOption, hasSelectedVariant) {
        ...
        if (hasSelectedVariant) {
          const variantImage = newVariant.querySelector('.variant-featured-image');
          const productImage = newVariant.querySelector('.product-featured-image');

          if (variantImage && variantImage.classList.contains('hidden')) {
        ...
        el.querySelectorAll(`li.open[data-option-name="${activeVariant.dataset.optionName}"]`).forEach((v) => {
          if (!v.isSameNode(selectedOption)) {
        ...
        const duplicateSelectedOption = el.querySelectorAll(
          `li[data-option-name="${selectedOption.dataset.optionName}"]`
        );

        if (duplicateSelectedOption.length > 1) {
          duplicateSelectedOption.forEach((v) => {
            if (hasSelectedVariant && v.classList.contains('deselect')) {
              v.classList.remove('deselect')
            }
            let parentNode = v.closest(".slick-slide");
            if (!parentNode.classList.contains("slick-cloned")) {
              v.classList.add("open");
              if (hasSelectedVariant) {
                const index = parseInt(parentNode.dataset.slickIndex, 10);
                if (!isNaN(index)) {
                  $(swatchSlider).slick("slickGoTo", index);
                }
              } else {
                $(swatchSlider).slick("slickGoTo", 0);
              }
          ...
          if (hasSelectedVariant && selectedOption.classList.contains('deselect')) {
      ...
      updateVariantOption(activeVariant, hasSelectedVariant = false);

      el.addEventListener("click", (e) => {
        const target = e.target.closest(".color__swatch li");
        if (target) updateVariantOption(target, hasSelectedVariant = true);
      });

      let resizeTimeout;
      window.addEventListener("resize", () => {
        clearTimeout(resizeTimeout);
        resizeTimeout = setTimeout(setupSwatchSlider, 200);
      });
